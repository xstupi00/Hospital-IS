...
<body>
 ...
 <div class="container">
  ...
  <form class="margin-bottom" method="post" action="add-examination.php?patient_id=<?php echo $Patient_ID;?>&hospid=<?php echo $Hospitalization_ID; ?>">
    <div class="container">
      ...
      <div class="form-group form-width  <?php set_input_error_success("Dateexam", $errors);?>">
        <label for="Dateexam"><span class="required-field">*&nbsp;</span>Date:</label>
        <?php create_input("text", "form-control", "Dateexam", "Set date in format 01/01/2018 (dd/mm/yyyy)", $arrayregex['Dateofregistration'], $arrayinputerrormessages["Dateofregistration"], isset($Dateexam) ? $Dateexam : "", "false", $errors, $arrayerrormessages, "readonly");  ?>
      </div>

      <div class="form-group form-width  <?php set_input_error_success("Type", $errors);?>">
        <label for="Type"><span class="required-field">*&nbsp;</span>Type:</label>
        <?php create_input("text", "form-control", "Type", "Enter type of examination", $arrayregex['Type'], $arrayinputerrormessages["Type"], isset($Type) ? $Type : "", "true", $errors, $arrayerrormessages, "");  ?>
      </div>


      <div class="form-group form-width">
        <label><span class="required-field">*&nbsp;</span>Medical record:</label>
        <textarea id="Recexam" name="Recexam" class="form-control inputstl" rows="10"  pattern="<?echo $arrayregex['Record'];?>" required=""></textarea>
      </div>
      ...
      <button type="submit" name="add-examination" class="btn btn-default" style="margin-top:20px;">Submit</button>
      </div>
  </form>
  </div>
</body>
...
<script>
  ...
  $( "#Dateexam" ).datepicker("setDate", today);
